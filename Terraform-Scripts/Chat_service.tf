...
# --- IAM Role (Commented out for AWS Learner Lab) ---
# REQUIREMENT NOTE: Section 2.1 requires separate IAM roles. 
# However, Learner Lab restricts IAM creation. 
# If running in Learner Lab, use the pre-existing 'LabRole' ARN instead for EC2/Lambda.
...
resource "random_id" "chat_bucket_suffix" {
  byte_length = 4
}


resource "aws_s3_bucket" "chat_storage" {
  bucket = "chat-service-storage-dev-${random_id.chat_bucket_suffix.hex}"
...
# 3. Database (RDS)
resource "random_password" "chat_db_pass" {
  length  = 16
  special = false
}
...
resource "aws_db_subnet_group" "chat_db_subnet_group" {
  ...
  tags = { Name = "Chat DB Subnet Group" }
}
...
resource "aws_db_instance" "chat_db" {
  ...
  username = "chatadmin"
  password = random_password.chat_db_pass.result
...
output "chat_db_password" {
  description = "Auto-generated password for Chat DB (Save this!)"
  value       = random_password.chat_db_pass.result
  sensitive   = true
}